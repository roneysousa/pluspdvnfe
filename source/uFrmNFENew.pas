...
begin
     ...
     Else
     begin}
         grdConsultar.Columns[11].Visible := false;
         grdConsultar.Columns[12].Visible := false;
     //End;
     //
     If (iNumNota = 0) Then
          Registro
...
Var
  ...
  M_VALOR_PIS, M_VALOR_COFINS : Double;
...
Var
   M_VLFRET, M_VLDESC, M_VALOR_ST, M_VALIPI : Double;
begin
     ...
     dmNFe.cdsNotaFiscalvalor_total_nota.AsFloat  := uFuncoes.Arredondar(
              ...
              + dmNFe.cdsNotaFiscalvalor_outras_despesas.AsFloat) - M_VLDESC)+M_VALOR_ST, 2);
end;
...
begin
     ...
     if uFuncoes.Empty(dbeCNPJCPFDest.Text) Then
      ...
      if uFuncoes.Empty(cmbUFDest.Text) Then
        begin
             Application.MessageBox('Selecione a UF do destinatário.','ATENÇÃO',
               MB_OK+MB_ICONINFORMATION+MB_APPLMODAL);
             PageControl1.ActivePageIndex := 2;
             cmbUFDest.SetFocus;
             Exit;
     ...
     If (dsCadastro.DataSet.State in [dsInsert, dsEdit]) Then
        begin
             ...
             begin
                  PageControl1.ActivePageIndex := 1;
                  Application.MessageBox('Campo CEP do Emitente está incompleto.','ATENÇÃO',
             ...
             begin
                  PageControl1.ActivePageIndex := 1;
                  Application.MessageBox('Campo Telefone do Emitente está incompleto.','ATENÇÃO',
                    MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
             ...
             begin
                  PageControl1.ActivePageIndex := 2;
                  Application.MessageBox('Campo Telefone do Destinatário/Remetente está incompleto.','ATENÇÃO',
                    MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
             ...
             begin
                  PageControl1.ActivePageIndex := 2;
                  Application.MessageBox('Campo logradouro do Destinatário/Remetente.'+#13
                                        +'Quantidade de caracteres inferior a 3.','ATENÇÃO',
                      MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
                  dbeLogradouroDest.SetFocus;
                  Exit;
             End;
             ...
             begin
                  PageControl1.ActivePageIndex := 2;
                  Application.MessageBox('Campo Bairro do Destinatário/Remetente.'+#13
                                        +'Quantidade de caracteres inferior a 3.','ATENÇÃO',
                    MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
                  dbeBairroDest.SetFocus;
                  Exit;
             ...
             begin
                  PageControl1.ActivePageIndex := 2;
                  Application.MessageBox('Campo CEP do Destinatário/Remetente está incompleto.','ATENÇÃO',
                    MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
                  edtCepDesc.SetFocus;
                  Exit;
             End;
             ...
             begin
                  PageControl1.ActivePageIndex := 2;
                  Application.MessageBox('Campo Cidade do Destinatário/Remetente.'+#13
                                        +'Quantidade de caracteres inferior a 3.','ATENÇÃO',
                    MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
                  dbeCidadeDest.SetFocus;
                  Exit;
     ...
     begin
         ...
         Application.MessageBox(PChar('Produto '+dmNFe.cdsItemsNotaFiscalCDS_DESCRICAO.AsString
              +' está sem unidade cadastrada.'),'ATENÇÃO', MB_OK+MB_ICONINFORMATION+MB_APPLMODAL);
         PageControl1.ActivePageIndex := 0;
         Exit;
     End;
     //
...
begin
    ...
    if (dsCadastro.DataSet.FieldByName('situacao').AsInteger = 5)
       AND (dsCadastro.DataSet.FieldByName('id').AsInteger = idUltimaNFe) Then   // and Not(dmNFe.GetExisteItensNota(idNotaFiscal))  to-do
         Begin
              If Application.MessageBox(PChar('Confirma exclusão de Nota?'+#13
                    +'Há não exclusão dessa NF-e deixa ela na situação de digitação.'),
                       ...
                       if not (dmDados.cdsProduto.IsEmpty) Then
                        begin
                             if uFuncoes.Empty(dmDados.cdsProdutoid_tributacao.AsString) Then
                                raise Exception.Create('Produto sem tributação cadastrada!!!');
                             //
   ...
   if (dsCadastro.DataSet.State in [dsInsert, dsEdit]) Then
       begin
           ...
           begin
                If uFuncoes.Empty(cmbNatureza.Text) then
                begin
                     Application.MessageBox('Selecione a natureza da operação.','ATENÇÃO',
                       MB_OK+MB_ICONINFORMATION+MB_APPLMODAL);
                     cmbNatureza.SetFocus;
                    Exit;
                End;
            ...
            If not (frmLocalizarCliente.cdsConsultar.IsEmpty) and (uFrmLocalizaCliente.M_TPCONSULTA = 'C') Then
             Begin
                       ...
                       if uFuncoes.Empty(frmLocalizarCliente.cdsConsultarcpf_cnpj.AsString) Then
                          raise Exception.Create('Cliente está sem CNPJ/CPF.');
              ...
              If not (frmLocalizarCliente.cdsFornecedores.IsEmpty) and (uFrmLocalizaCliente.M_TPCONSULTA = 'F') Then
                  begin
                       ...
                       if uFuncoes.Empty(frmLocalizarCliente.cdsFornecedorescpf_cnpj.AsString) Then
                          raise Exception.Create('Fornecedor está sem CNPJ/CPF.');
        ...
        If (dmNFe.cdsNotaFiscaldestinatario_tipopessoa.AsString = 'F') and not uFuncoes.Empty(aCNPJCPF) Then
          begin
               if (Length(aCNPJCPF) < 11) Then
                begin
                     MessageDlg('Quantidade de caracteres inválidos!', mtWarning, [mbOk], 0);
                     dbeCNPJCPF.SetFocus;
                     Exit;
                End;
          End;
               ...
               If (dmDados.ProcurarValor(aCNPJCPF,'cpf_cnpj','clientes')) Then
                ...
                begin
                    If (dmDados.ProcurarValor(aCNPJCPF,'cpf_cnpj', 'fornecedores')) then
                     ...
                     Else
                     begin
                          Application.MessageBox(PChar('CPF/CNPJ não cadastrado.'),
...
begin
     ...
     begin
          if not (dmDados.cdsFornecedortransportadora.AsBoolean) Then
           begin
                dbeTransportadora.SetFocus;
                raise Exception.Create('Fornecedor não é uma transportadora.');
           End;
...
Var
    M_NRNOTA, iUsuario, M_CDCIDDES, M_CDCIDRET, M_CDITEM, idNFeEmpresa, idMaxNFeEmpresa : Integer;
    M_VLNOTA : Double;
    aOperacao : String;
    M_CODNCM : string[8];
...
begin
     ...
     begin
          PageControl1.ActivePageIndex := 0;
          Application.MessageBox(PChar('Selecione uma opção no campo consumidor final.'),
                           'ATENÇÃO', MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
     ...
     begin
          PageControl1.ActivePageIndex := 0;
          Application.MessageBox(PChar('Selecione o destino da operação!'),
                           'ATENÇÃO', MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
     ...
     If uFuncoes.Empty(cmbTipoContribuinte.Text) Then
      ...
      if (dsCadastro.DataSet.FieldByName('ind_inscricao_estadual_dest').AsInteger = 1)
        and uFuncoes.Empty(dsCadastro.DataSet.FieldByName('destinatario_ie').AsString) Then
         begin
              PageControl1.ActivePageIndex := 2;
              Application.MessageBox(PChar('Digite a Inscrição Estadual do Destinatário/Remetente!'),
                           'ATENÇÃO', MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
     ...
     if not (VerificarNCMProduto(dmNFe.cdsItemsNotaFiscal)) Then
       begin
            PageControl1.ActivePageIndex := 0;
            M_CODNCM := inputbox('Código NCM', dmNFe.cdsItemsNotaFiscalCDS_DESCRICAO.AsString,'');
       ...
       //
        ...
        begin
             ...
             Application.MessageBox(PChar('Selecione a UF da placa do veículo.'),
     ...
     Try
          if dsCadastro.DataSet.State in [dsInsert] Then
            ...
            begin
                ...
                begin
                     If not (dmNFe.SalvarRegistroNotaReferencia(dmNFe.cdsNotaFiscalid.AsInteger, dmNFe.cdsListaNT_Ref)) Then
                      begin
                           ShowMessage('Erro ao tentar gravar nota(s) de referências.');
                           dmDados.Rollback('Erro ao tentar gravar nota(s) de referências.');
                      End;
                End
                Else
               ...
               { if (FNRVENDA <> 0) Then
                  begin
                       If not ( DM.SetNumNfeVenda(dsCadastro.DataSet.FieldByName('numero_nota_fiscal').AsInteger, FNRVENDA, 1)) Then
                             Application.MessageBox(PChar('Não foi possível associar NF-e a venda selecionada.'),
            ...
            End;
            //
            Application.MessageBox(PChar(uFuncoes.MSG_OK),'ATENÇÃO', MB_OK+MB_ICONINFORMATION+MB_APPLMODAL);
            //
            dmNFe.LimparListaVenda;  
            tratabotoes;
            PageControl1.ActivePageIndex := 0;
      ...
      if not uFuncoes.Empty(dbeCidadeEmissor.Text) and not uFuncoes.Empty(cmbUFEmitente.Text) Then
        begin
             if not dmNFe.GetVerificarDescricaoCidade(dbeCidadeEmissor.Text, cmbUFEmitente.Text) Then
              begin
                   Application.MessageBox(PChar('Cidade não cadastrado na UF.'),
                            'ATENÇÃO', MB_OK+MB_ICONINFORMATION+MB_APPLMODAL);
                   dbeCidadeEmissor.SetFocus;
      ...
      and not uFuncoes.Empty(dbeCNPJ.Text) Then
       begin
            If (Length( uFuncoes.Alltrim(dbeCNPJ.Text)) < 11) Then
             begin
                   MessageDlg('Tamanho de CNPJ/CPF inválido!!!', mtInformation, [mbOk], 0);
                   dbeCNPJ.Clear;
                   dbeCNPJ.SetFocus;
                   Exit;
             End;
    ...
    If not (dsCadastro.DataSet.IsEmpty)
     and (dsCadastro.DataSet.State in [dsbrowse]) Then
      begin   
           ...
           begin
                If Application.MessageBox('Deseja alterar status para digitação da NF-e?',
                    'ATENÇÃO', MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON2+MB_APPLMODAL) = idYes then
                    begin
           ...
           begin
                ...
                Application.MessageBox(PChar('Situação da nota: '+aSituacao),
                       'ATENÇÃO', MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
                Exit;
           End;
           //
     ...
     and not uFuncoes.Empty(dbeNumero.Text) Then
      begin
          ...
          if (DMDados.VerificaExisteNFeEmpresa(uFrmPlusPdvNfe.idEmpresa,  dbeNumero.Text)) Then
           begin
                Application.MessageBox(PChar('Número de nota já cadastrado para esse emitente.'),
                       'ATENÇÃO', MB_OK+MB_ICONINFORMATION+MB_APPLMODAL);
                dbeNumero.Clear;
                dbeNumero.SetFocus;
                Exit;
...
begin
     ...
     {If (FNRVENDA <> 0) Then
      begin
          Application.MessageBox(PChar('Uma venda já foi importada!!!'),
                       'ATENÇÃO', MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
          Exit;
      End;}
      //
                      ...
                      begin
                           ...
                           begin
                               ...
                               begin
                                    ...
                                    dmNFe.cdsNotaFiscalnum_coo.AsString     := dmNFe.cdsVendaItensNFEcoo.AsString;
                                    //
     ...
     if not uFuncoes.Empty(dbeInscricaoEstadual.Text) AND
       ...
       begin
           { If not Inscricoes.Inscricao(dbeInscricaoEstadual.Text, dsCadastro.DataSet.FieldByName('destinatario_uf').AsString) Then
             begin
                  Application.MessageBox(PChar('Número inscrição estadual inválida.'),
                            'ATENÇÃO', MB_OK+MB_ICONINFORMATION+MB_APPLMODAL);
           ...
           if not uFuncoes.Empty(edtUnidade.Text) Then
            begin
                 If not (dmDados.ProcurarValor(edtUnidade.Text, 'id', 'unidades')) Then
                  begin
                       Application.MessageBox(PChar('Código de Unidade não cadastrado.'),
                         'ATENÇÃO', MB_OK+MB_ICONINFORMATION+MB_APPLMODAL);
                       edtUnidade.Clear;
                       edtUnidade.SetFocus;
                       Exit;
...
begin
   ...
   { TODO -oT2Ti -cEstoque : Devemos permitir que o usuário altere dados da Nota importada? }
 ...
 try
  ...
  begin
    ...
    // Verifica CNPJ do Emitente ae igual a da Empresa do usuário
    dmDados.FilterCDS(dmDados.cdsEmpresa, fsInteger, InttoStr( uFrmPlusPdvNfe. idEmpresa));
    ...
    if uFuncoes.Empty(dmDados.cdsEmpresacnpj.AsString) Then
     ...
     End;
         ...
         begin
              Raise Exception.Create('Arquivo XML não pertence a empresa: '+ dmDados.cdsEmpresarazao_social.AsString+'.'+#13+'Importação não realizada.');
         End;
    ...
    begin
      ...
      begin
        ...
        begin
             Application.MessageBox('NFe já cadastrada.', 'Mensagem do Sistema', MB_OK + MB_ICONEXCLAMATION);
             Break;
        End;
        //
        inNF := Ide.nNF;
        ...
        { TODO -oT2Ti -cEstoque : No manual da NF-e encontramos o refere aos cupons fiscais sendo
          filho do grupo referente às notas referenciadas, mas no banco de dados nós vinculamos a
          tabela de cupons fiscais diretamente à tabela NFE_CABECALHO. Isso está correto? }
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              If not (DMDados.ProcurarValorInt(aProduto, 'id', 'produtos')) Then
                   Application.MessageBox(PChar('Código de produto '+aProduto+' não cadastrado.'), 'Mensagem do Sistema', MB_OK + MB_ICONINFORMATION);
              ...
              { TODO -oT2Ti -cEstoque : Como podemos destacar na grid o produto que não existe na base? }
              { TODO -oT2Ti -cEstoque : O que vamos fazer caso o produto não exista na base? Cadastrar automaticamente? Como? }
              ...
              begin
                ...
                { TODO -oT2Ti -cEstoque : As informações referentes ao PIS abaixo estão completas? }
                with PIS do
                begin
                ...
                { TODO -oT2Ti -cEstoque : As informações referentes ao COFINS abaixo estão completas? }
                with COFINS do
                begin
                ...
                begin
                  ...
                  begin
                    ...
                    // TODO -oT2Ti -cEstoque : Implementar Detalhamento Específico de Medicamento. }
            ...
            { TODO -oT2Ti -cEstoque : Implementar Detalhamento Específico de Combustíveis. }
        ...
        {0- Por conta do emitente;                C
         1- Por conta do destinatário/remetente;  F
        ...
        { TODO -oT2Ti -cEstoque : É possível que exista mais de um reboque por transporte? Se sim, o que devemos fazer com nossa tabela no banco de dados? }
        ...
        begin
             ...
             Open;
        End;}
        { TODO -oT2Ti -cEstoque : É possível que exista mais de um volume por transporte? Se sim, o que devemos fazer com nossa tabela no banco de dados? }
   ...
   if (dsCadastro.DataSet.State in [dsInsert, dsEdit])
       ...
       begin
             if not uFuncoes.ValidarEMail(dbeEmailDest.Text) Then
              begin
                   Application.MessageBox(PChar('Email inválido.'),
                            'ATENÇÃO', MB_OK+MB_ICONEXCLAMATION+MB_APPLMODAL);
            ...
            begin
                ...
                if (Length(aNumPlaca) < 7) Then
                 begin
                      ...
                      raise Exception.Create('Número da placa é inválido!!!');
                 End;
            End;
      End;
end;
