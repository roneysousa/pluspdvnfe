...
type
  ...
  public
    ...
    Function GerPMCProduto(idProduto : Integer): Double;
    Function GetCNPJEmpresa(idEmpresa : Integer): String;
  end;
...
begin
    Result := 0;
    aCnpjEmitente := GetCNPJEmpresa(idEmpresa);
    ...
    texto := 'Select Max(numero_nota_fiscal) as total from nota_fiscal where (id_empresa = :pemitente) and ';
    texto := texto + '(indicador_do_emitente = '+QuotedStr('0')+') and (situacao <> 99) and (modelo = 8) and (emitente_cnpj = '+QuotedStr(aCnpjEmitente) +') ';
    QrAux := TSqlquery.Create(nil);
    with QrAux do
     try
          SQLConnection := sqlConexao;
...
begin
    ...
    texto := 'Select id, numero_nota_fiscal, id_empresa from nota_fiscal where (numero_nota_fiscal = :pnumeronota) and (id_empresa = :pemitente) and (indicador_do_emitente = '+QuotedStr('0')+') and (modelo = 8) ';
    QrAux := TSqlquery.Create(nil);
    with QrAux do
     try
          SQLConnection := sqlConexao;
...
function TdmDados.GerPMCProduto(idProduto: Integer): Double;
var qraux : TSQLquery;
    texto : string;
begin
  Result := 0;
  texto := 'Select id, pmc from produtos where (id = :pid)';
  QrAux := TSQLquery.Create(nil);
  with QrAux do
    try
      SQLConnection := sqlConexao;
      ...
      //
      if not (IsEmpty) then
         if (FieldByname('pmc').AsFloat > 0) Then
               result := FieldByname('pmc').AsFloat;
    finally
      free;
    end;
end;

function TdmDados.GetCNPJEmpresa(idEmpresa: Integer): String;
var qraux : TSQLquery;
    texto : string;
begin
    texto := 'Select id, cnpj from empresas where (id  = :pLoja) ';
    QrAux := TSqlquery.Create(nil);
    with QrAux do
     try
       SQLConnection := sqlConexao;
       sql.Add(texto);
       Params[0].AsInteger := idEmpresa;
       Open;
       //
       if not (IsEmpty) Then
          Result := uFuncoes.Alltrim(FieldByName('cnpj').AsString);
       finally
         free;
       end;
end;
