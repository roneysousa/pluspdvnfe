...
type
  TFrmCadItemNota = class(TForm)
    ...
    Label42: TLabel;
    Label43: TLabel;
    dbeBCIcmsSimples: TDBEdit;
    edtAliquotaICMSSimples: TDBEdit;
    dbeValorIcmsSimples: TDBEdit;
    dbePercReducaoSimples: TDBEdit;
    ...
    procedure cmbSimplesExit(Sender: TObject);
    procedure dbeBCIcmsSimplesExit(Sender: TObject);
    procedure edtAliquotaICMSSimplesExit(Sender: TObject);
    procedure dbePercReducaoSimplesExit(Sender: TObject);
    procedure dbePercReducaoSimplesKeyPress(Sender: TObject;
      var Key: Char);
    procedure dbeBCIcmsSimplesKeyPress(Sender: TObject; var Key: Char);
    procedure edtAliquotaICMSSimplesKeyPress(Sender: TObject;
      var Key: Char);
  private
    ...
    procedure CNSON_Campos(aCodigo : String);
    procedure Tributacao_simples();    
  public
    { Public declarations }
  end;
...
begin
     ...
     if (dsCadastroItem.DataSet.FieldByName('CDS_ALIQICMS').AsFloat > 0) Then
      begin
          if not uFuncoes.Empty(dsCadastroItem.DataSet.FieldByName('CDS_CST').AsString) and (tsbICMS.tabVisible) Then
           begin
           ...
           Else
           begin
                if (tsbSimples.TabVisible) Then
                begin
                     If (dsCadastroItem.DataSet.FieldByName('CDS_PEREDUCAO').AsFloat > 0) Then
                          fPercReducaoBC_ICMS := dsCadastroItem.DataSet.FieldByName('CDS_PEREDUCAO').AsFloat;
                     //
                     if not uFuncoes.Empty(dsCadastroItem.DataSet.FieldByName('CDS_CSOSN').AsString)
                       and (dsCadastroItem.DataSet.FieldByName('CDS_CSOSN').AsInteger = 900) then
                     begin
                          if (fPercReducaoBC_ICMS > 0) Then
                              dsCadastroItem.DataSet.FieldByName('CDS_BCICMS').AsFloat := uFuncoes.Arredondar(M_VLSUBT - (M_VLSUBT * (fPercReducaoBC_ICMS/100)),2)
                          Else
     ...
     Else
     begin
         dsCadastroItem.DataSet.FieldByName('CDS_BCICMS').AsFloat := 0;
...
begin
     ...
     begin
          ...
          if not uFuncoes.Empty(dsCadastroItem.DataSet.FieldByName('CDS_CSOSN').AsString) Then
               CNSON_Campos(dsCadastroItem.DataSet.FieldByName('CDS_CSOSN').AsString);
          //
          ...
          begin
             ...
             begin
                 // IPI
                 if uFuncoes.Empty(dsCadastroItem.DataSet.FieldByName('CDS_IPI_CST').AsString) Then
                    dsCadastroItem.DataSet.FieldByName('CDS_IPI_CST').AsInteger := 53;
                 // PIS
                 if uFuncoes.Empty(dsCadastroItem.DataSet.FieldByName('CDS_CST_PIS').AsString) Then
                    dsCadastroItem.DataSet.FieldByName('CDS_CST_PIS').AsInteger := 99;
                 // Cofins
                 if uFuncoes.Empty(dsCadastroItem.DataSet.FieldByName('CDS_CST_COFINS').AsString) Then
                    dsCadastroItem.DataSet.FieldByName('CDS_CST_COFINS').AsInteger := 99;
             End
             Else
             begin
...
begin
     if (tsbICMS.tabVisible) then
     begin
         CamposReducaoICMS(false);   // aqui
         if (dsCadastroItem.DataSet.FieldByName('CDS_CST').AsInteger = 20) Then
...
procedure TFrmCadItemNota.CNSON_Campos(aCodigo: String);
begin
     gbxICMSSimples.Visible := False;
     if (aCodigo = '900') Then
         gbxICMSSimples.Visible := True;
end;

procedure TFrmCadItemNota.cmbSimplesExit(Sender: TObject);
begin
     if (dsCadastroItem.DataSet.State in [dsInsert, dsEdit]) then
      begin
          CNSON_Campos(dsCadastroItem.DataSet.FieldByName('CDS_CSOSN').AsString);
...
procedure TFrmCadItemNota.Tributacao_simples;
begin
     If (dsCadastroItem.DataSet.State in [dsInsert, dsEdit]) then
     begin
          if not uFuncoes.Empty(dsCadastroItem.DataSet.FieldByName('CDS_CSOSN').AsString)
             and (dsCadastroItem.DataSet.FieldByName('CDS_CSOSN').AsInteger <> 900) Then
          begin
              dsCadastroItem.DataSet.FieldByName('CDS_ALIQICMS').AsFloat := 0;
              dsCadastroItem.DataSet.FieldByName('CDS_BCICMS').AsFloat   := 0;
              dsCadastroItem.DataSet.FieldByName('CDS_VLICMS').AsFloat   := 0;
          End;
     End;
...
procedure TFrmCadItemNota.dbeBCIcmsSimplesExit(Sender: TObject);
begin
     If (dsCadastroItem.DataSet.State in [dsInsert, dsEdit]) then
        Calcular;
end;

procedure TFrmCadItemNota.edtAliquotaICMSSimplesExit(Sender: TObject);
begin
     If (dsCadastroItem.DataSet.State in [dsInsert, dsEdit]) then
        Calcular;
end;

procedure TFrmCadItemNota.dbePercReducaoSimplesExit(Sender: TObject);
begin
     If (dsCadastroItem.DataSet.State in [dsInsert, dsEdit]) then
         Calcular;
end;

procedure TFrmCadItemNota.dbePercReducaoSimplesKeyPress(Sender: TObject;
  var Key: Char);
begin
     if (dsCadastroItem.DataSet.State in [dsInsert, dsEdit]) Then
      If (key = #13) Then
...
procedure TFrmCadItemNota.dbeBCIcmsSimplesKeyPress(Sender: TObject;
  var Key: Char);
begin
     if (dsCadastroItem.DataSet.State in [dsInsert, dsEdit]) Then
      If (key = #13) Then
...
procedure TFrmCadItemNota.edtAliquotaICMSSimplesKeyPress(Sender: TObject;
  var Key: Char);
begin
     if (dsCadastroItem.DataSet.State in [dsInsert, dsEdit]) Then
      If (key = #13) Then
